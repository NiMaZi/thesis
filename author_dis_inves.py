...
f=open(homedir+"/results/statistics/authorinfo_dependence.json",'r',encoding='utf-8')
...
for i in range(0,10000):
    try:
        ...
        continue
    try:
        myBucket.download_file("yalun/Dependence/authors"+str(7*i)+".json",homedir+"/temp/tmpjson.json")
    except:
        continue
...
res=list(set(res))
f=open(homedir+"/results/logs/authordis.csv",'w')
wt=csv.writer(f)
for r in res:
	wt.writerow(r)
f.close()
...
f=open(homedir+"/results/logs/authordis.txt",'w')
