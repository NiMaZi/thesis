...
def load_sups():
	...
	bucket=get_bucket()
	bucket.download_file("yalun/results/ontology/cc2vid_eegrdc_enriched_2nd.json",homedir+"/results/ontology/cc2vid_eegrdc_enriched_2nd.json")
	f=open(homedir+"/results/ontology/cc2vid_eegrdc_enriched_2nd.json",'r')
	cc2vid=json.load(f)
	f.close()
	return cc2vid

def build_model(_input_dim=33934,_hidden_dim=512,_drate=0.5):
	model=Sequential()
...
def train_on_batch_S3(_model,_source,_volume,_bcount,_batch,_mbatch,_epochs=5):
	...
	for i in range(21216,_volume):
		...
		if len(sample_list)>=_batch:
			...
			updata=s3f.read()
			bucket.put_object(Body=updata,Key="yalun/results/models/MLPsparse_1hidden_eegrdc_e2nd.h5")
			s3f.close()
			logf=open(homedir+"/results/logs/bow_training_log_eegrdc_e2nd.txt",'a')
