...
def load_sups():
	...
	cc2vid=json.load(f)
	f.close()
	f=open(homedir+"/temp/cc2vid_disease.json",'r')
...
def test_on_doc_S3(_model,_volume,_threshold=0.0):
	...
	for i in range(0,_volume):
		...
		Y_test=np.ceil(N_test[:,len(cc2vid):])[0].astype(int)
...
if __name__=="__main__":
	...
	while i<=130:
		homedir=os.environ['HOME']
		logf=open(homedir+"/results/logs/bow_score_sem.txt",'a')
		model_name="MLPsparse_1hidden_sem_"+str(i)
		volume=100
		logf.write("%s,%d\n"%(model_name,volume))
