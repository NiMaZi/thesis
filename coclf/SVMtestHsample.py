				...
				for i1 in range(0,len(abs_key_list)):
					for i2 in range(i1,len(abs_key_list)):
						...
						if b_key in pred_dict_rbf.keys():
							pred_dict_rbf[b_key]+=(1.0*(disa1b+disa2b))/(2.0*disa1b*disa2b)
						else:
							pred_dict_rbf[b_key]=(1.0*(disa1b+disa2b))/(2.0*disa1b*disa2b)
			ordered_list=sorted(pred_dict_rbf,key=pred_dict_rbf.get)
			pred_set_rbf=set(ordered_list[int(confidence*len(ordered_list)):])
